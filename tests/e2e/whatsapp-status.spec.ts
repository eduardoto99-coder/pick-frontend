...
const candidateId = `e2e-whatsapp-candidate-${runId}`;
const candidateName = `Candidata WhatsApp ${runId}`;
...
async function fetchFirstInterest() {
  const payload = await fetchJson(`${apiUrl}/interests/search`, {
    method: "GET",
    headers: { "content-type": "application/json" },
  });
  const interests = Array.isArray(payload?.interests) ? payload.interests : [];
  if (!interests.length) {
    throw new Error("No pudimos obtener intereses aprobados para la prueba.");
  }
  return interests[0] as { id: string; label: string };
}
...
async function seedCandidateProfile() {
  const payload = {
    ...
    profile: {
      ...
      bio: "Perfil de prueba para validar el seguimiento de WhatsApp en sugerencias.",
...
if (cognitoIds.length) {
  ...
  whatsapp = db.whatsapp_click_events.deleteMany({
    $or: [{ requesterId: { $in: cognitoIds } }, { partnerId: { $in: cognitoIds } }],
  }).deletedCount;
}
if (emails.length) {
  consents = db.consents.deleteMany({ email: { $in: emails } }).deletedCount;
}
printjson({ users, consents, whatsapp });
`;
...
test.beforeAll(async () => {
  ...
  const resolved = await fetchFirstInterest();
  interestId = resolved.id;
...
test("marks WhatsApp intros as connected", async ({ page }) => {
  testEmail = `pick.e2e.whatsapp.${runId}@example.com`;
  const password = "PickE2E123!";
  const displayName = `E2E WhatsApp ${runId}`;
  const bioText =
    "Estoy probando Pick con seguimiento de mensajes por WhatsApp.";
  ...
  await page.getByLabel(/LinkedIn/i).fill("https://www.linkedin.com/in/e2e-whatsapp");
  await page.getByLabel(/Instagram/i).fill("https://instagram.com/e2e.whatsapp");
  ...
  const newSection = page.getByTestId("matches-new");
  const connectedSection = page.getByTestId("matches-connected");

  await expect(newSection.getByText(candidateName)).toBeVisible({ timeout: 30_000 });
  await expect(connectedSection.getByText(candidateName)).toHaveCount(0);

  const candidateCard = newSection.getByTestId(`match-card-${candidateId}`);
  await candidateCard.getByRole("button", { name: /whatsapp/i }).click();

  await expect(connectedSection.getByText(candidateName)).toBeVisible({ timeout: 30_000 });
  await expect(newSection.getByText(candidateName)).toHaveCount(0);

  await page.reload();
  await expect(page.getByTestId("matches-connected").getByText(candidateName)).toBeVisible({
    timeout: 30_000,
  });
  await expect(page.getByTestId("matches-new").getByText(candidateName)).toHaveCount(0);
});
